...
if(isset($_POST['submit'])){
   ...
   if($select_admin->rowCount() > 0){
      ...
      if($pass != $cpass){
         $message[] = 'a senha nao deu certo po';
      }else{
         $insert_admin = $conn->prepare("INSERT INTO `admins`(name, password) VALUES(?,?)");
         $insert_admin->execute([$name, $cpass]);
         $message[] = 'sucessssssoo';
      }
   }
...
<head>
   ...
   <link rel="stylesheet" href="../ss/admin_style.css">
...
<section class="form-container">
   ...
   <form action="" method="post">
      <h3>registrar</h3>
      <input type="text" name="name" required placeholder="coloque seu nome de usuário" maxlength="20"  class="box" oninput="this.value = this.value.replace(/\s/g, '')">
